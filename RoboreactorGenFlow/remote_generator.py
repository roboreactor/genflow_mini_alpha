           ...
           def get_serial_port(self):   # Getting the serial devices name 
               ...
               try:
                    ...
                    for i in range(0,len(serial_port)):
                                port_and_name = subprocess.check_output(" python3 -m serial.tools.list_ports -v",shell=True)
...
def device_info():
       ...
       data_host = {'email':'kornbot380@hotmail.com',"serial_devices":serial_devices,"Vision_system":cam_device,"Audio_system":audio_device,"Host_name":host_name,"IP":ip,'Local_ip':localip}
...
t1 = threading.Thread(target=app.run(debug=True,threaded=True,host="0.0.0.0",port=9060)).start() 
